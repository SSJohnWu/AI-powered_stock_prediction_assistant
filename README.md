...
2. **🤖 雙模型預測與對比**：
   ...
   - **未來走勢預測**：雙模型均支援**自迴歸滾動預測 (Autoregressive Recursive Forecast)**，可向後預測**未來 20 個交易日**的股價趨勢。
3. **📈 互動式圖表可視化**：採用 `Plotly` 繪製精美的互動式折線圖，對比實際歷史收盤價、測試集預測值，以及向後延伸的未來 20 天模型預測走勢。
4. **📉 模型表現精準評估**：
   - 計算並呈現兩大模型的 **MAE (平均絕對誤差)**、**RMSE (均方根誤差)** 與 **方向準確率**。
   - 提供最後 20 天歷史預測對比表以及未來 20 天的預測數值細節表。
5. **💬 Gemini 金融分析師報告**：透過最新版 `google-genai` SDK，串接 `gemini-2.5-flash` 模型，結合模型歷史評估指標與未來 20 天預測走勢，自動生成 200 字內的專業股市分析與買賣建議。
